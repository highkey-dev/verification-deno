import { generatePublicKeyCredentialCreationOptions, parseAuthenticatorData, sha256 } from "./util.ts";
import * as storage from "./../storage/persistentKeyStore.ts";
import * as cache from "./../storage/challengeCache.ts";

import { GenericAttestation } from "../models/custom/GenericAttestation.ts";
import { ClientDataJSON } from "../models/fido/ClientDataJSON.ts";
import { User } from "../models/custom/User.ts";

import { isTPMAttestation, TPMVerify, TPMStmt } from "../models/fido/Attestation Statement Format/TPM.ts"
import { isPackedAttestation, PackedVerify, PackedStmt } from "../models/fido/Attestation Statement Format/Packed.ts"
import { isAndroidKeyAttestation, AndroidKeyVerify } from "../models/fido/Attestation Statement Format/Android Key.ts"
import { isAndroidSafetyNetAttestation, AndroidSafetyNetVerify, AndroidSafetyNetStmt } from "../models/fido/Attestation Statement Format/Android Safety Net.ts"
import { isFIDOU2FAttestation, FIDOU2FVerify } from "../models/fido/Attestation Statement Format/FIDO U2F.ts"
import { isNoneAttestation, NoneVerify } from "../models/fido/Attestation Statement Format/None.ts"
...
import { ErrorMessage } from "../models/custom/ErrorMessage.ts";
...
export function registerKey(keyCredentialObject: { [key: string]: any }, userId:string): ErrorMessage {
	const { authenticatorSelection } = generatePublicKeyCredentialCreationOptions("", "");
	...
	// TODO: disable until cross-origin creation is supported: https://github.com/w3c/webauthn/issues/1656
	// if (process.env.BASE_URL && !process.env.BASE_URL.includes("dev.highkey.test") && !(clientData.origin === process.env.BASE_URL)) {
	// 	return {
	// 		status: 403,
	...
	const attestation: GenericAttestation = CBOR.decode(Buffer.from(keyCredentialObject.attestationObject, 'base64'));
	...
	if (authenticatorSelection.userVerification === "required" && !(authenticatorData.flags & 4)) {
		return {
			status: 403,
	...
	*/
	...
	return {status: 200, text:"Registration successful!", credential: credential};
}
